...
export class PineoService {

  constructor(private http: HttpClient) { }

  obtenerClientes() {
    return this.http.get<Cliente[]>(`${environment.api}/api/cliente/pedido`, { headers: { 'Authorization': `Bearer ${token}` } }).toPromise();
  }

  postearCoordenadas(coordenada: Coordenada) {
    return this.http.post(`${environment.api}/api/cliente/coordenadas`, coordenada).toPromise();
  }
